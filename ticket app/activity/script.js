...
function selectFilter(e){
    if(e.target.classList.contains("active-filter")){
        ...
        ticketContainer.innerHTML="";
        let filteClicked = e.target.classList[1];
        loadSelectedTickets(filteClicked);
    }
...
function openTicketModal(e){
    ...
    <div class="ticket-filters">
        ...
        <div class="ticket-filter orange"></div>    
    </div>`;
...
function handleKeyPress(e){
    if(e.key == "Enter" && isTextTyped && e.target.textContent){
      ...
      let ticketInfoObject = {
            ticketFilter: filterSelected , ticketValue: e.target.textContent, ticketId:ticketId 
        };
